...
const GEMINI_API_KEY = 'AIzaSyAB7poLTB3IrEp4q8G1Ffi35-J-vPQ7TOk';
...
app.get('/api/admin/feedback/report', authMiddleware(['admin']), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT f.*, u.name, u.division, u.panel 
      FROM feedback f 
      JOIN users u ON f.user_id = u.id 
    ...
    if (feedbacks.length === 0) {
      return res.json({ report: "No feedback available to analyze." });
    }

    let feedbackText = "";
    feedbacks.forEach((f, index) => {
      feedbackText += `${index + 1}. [Division: ${f.division || 'N/A'}, Panel: ${f.panel || 'N/A'}, Category: ${f.category}] Feedback: ${f.feedback_text}\n`;
      if (f.additional_notes) {
        feedbackText += `   Additional Notes: ${f.additional_notes}\n`;
      }
      feedbackText += "\n";
    });
    ...
    const prompt = `Analyze the following student feedback and generate a summary report. Identify common issues, complaints, and positive notes. 
    For each issue or point identified, mention which Division and Panel it came from if available in the data.

At the very top of your response, include the following counts on separate lines:
POSITIVE_COUNT: [number]
NEGATIVE_COUNT: [number]
NEUTRAL_COUNT: [number]
SUMMARY: [Brief 1-2 sentence summary]

Then after a separator "---", provide the detailed report in plain text. Group by problems/issues and mention the associated Division/Panel. Do NOT use markdown symbols like # or **. Use simple spacing and capitalization for structure.
...
${feedbackText}`;
    ...
    res.status(500).json({ message: 'Server error or Gemini API error', error: err.message });
  }
});
